...
const Mypage_4 = () => {
  ...
  const handleMypageBoardList = async () => {
      try {
          const response = await axios.get(`${process.env.REACT_APP_API_URL}/boards/my-bookmarks`, {
            crossDomain: true,
  ...
  return (
    <div className="mypage_container">
      <div className="mypage_top">북마크한 게시글만 모았어요 !</div>
        ...
        {mypageboardData.map((item, index) => (
          <Link to={`/board/${item.boardId}`} key={item.boardId} style={{ textDecoration: "none", color : "#000000"}}>          
            <div className="mypage_2_content">
              <div className="mypage_2_content_title">{item.title.length > 10 ? item.title.substring(0, 10) + "..." : item.title}</div>
              <div className="mypage_2_content_info">
                <div className="mypage_2_content_date">작성일 {item.createdAt.split("T")[0]}</div>
                <div className="mypage_2_content_date">수정일 {item.modifiedAt.split("T")[0]}</div>
